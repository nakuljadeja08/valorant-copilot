...
/** Narrowest a chart will draw. A 320px phone leaves ~235px inside a panel, so
    this must sit below that or the SVG overflows and the page scrolls sideways. */
export const MIN_CHART_W = 220;

/** Container width, so charts size to the viewport instead of guessing.

    A callback ref rather than an effect on mount: the plot container unmounts
    whenever a chart flips to its table twin, and an effect with [] deps would
    never observe the remounted node. */
